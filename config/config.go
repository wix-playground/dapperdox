...
type config struct {
	...
	SkipOnLoadError    bool        `env:"SKIP_ON_LOAD_ERROR" flag:"skip-on-load-error" flagDesc:"When set to true, if the loading of a specification errors due to connectivity, skip to the next specification instead of exiting with a hard failure."`
